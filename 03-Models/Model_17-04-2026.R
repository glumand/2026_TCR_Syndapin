...
reactions <- NULL %>%
  ...
  # addReaction("TCRL_Surface", "TCR_Surface + free_ligand", rate = "k_L * TCRL_Surface * ligand") %>% 
  # addReaction("TCR_Surface + free_ligand", "TCRL_Surface", rate = "k_LR * TCR_Surface * free_ligand * ligand") %>%
...
trafo <- NULL %>% 
  ...
  insert("x~y", x="TCR_Surface", y="TCR_Surface_CTRL", conditionMatch = "0_shCTRL") %>%
...
outframe$value <- outframe$value + 1482
waterfall <- plotValues(outframe[1:400])
print(waterfall)
...
save(file = file.path(.resultsFolder, "23042026_profile_results.Rdata"), profiles)
plotProfilesAndPaths(profiles, whichpars = c("k_B", "offset_Surface_full", "offset_Surface_sec", "offset_Total"))
plotProfilesAndPaths(profiles, whichpars = c("scale_Surface", "scale_Total"))
plotProfilesAndPaths(profiles, whichpars = c("TCR_Endo_CTRL", "TCR_Endo_CTRL"))
